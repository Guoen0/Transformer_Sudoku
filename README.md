# Transformer 做数独任务
这个项目用 Transformer 18M 参数模型对于 9*9 数独任务正确率达 93.5%。模型通过回溯算法的动作轨迹数据学会类似 CoT 的行为来解开谜题。
...
## 经验
- 训练数据必须使用由回溯算法生成的带有擦除动作的动作轨迹序列，否则模型无法收敛。尝试过逐行逐列的序列，随机填空的序列，以及由回溯算法生成的不带擦除动作的序列，全无法收敛。带擦除动作的回溯动作轨迹是解开谜题必要的 CoT。
- 训练过程中，模型会自动先掌握低级别的数组（隐藏数量少），后掌握高级别数独。