...
# ------------------------------------------------------------
is_continue_train = False       # 是否加载模型继续训练
suffix = "9x"                  # 选择 6x、9x 环境和模型，6x不需要验证小方格
# ------------------------------------------------------------
...
if not is_continue_train:
    ...
    max_length = 500 if suffix == "9x" else 60 if suffix == "6x" else 10
    dff = d_model * 2

    num_iterations = 20000 if suffix == "9x" else 5000 if suffix == "6x" else 500
...
if env.size == 9:
    evaluation_data = pickle.load(open("data/evaluation_data_9x5k.pkl", "rb"))
    training_data = pickle.load(open("data/training_data_9x100k.pkl", "rb"))
elif env.size == 6:
    evaluation_data = pickle.load(open("data/evaluation_data_6x5k.pkl", "rb"))
    training_data = pickle.load(open("data/training_data_6x20k.pkl", "rb"))
elif env.size == 3:
    evaluation_data = pickle.load(open("data/evaluation_data_3x1k.pkl", "rb"))
